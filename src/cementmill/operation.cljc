(ns cementmill.operation
  "OperationActor -- one cement-mill operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor (Cement
  Mill Advisor) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the Kiln Governor (:govern) and the
  ...
  One graph run = one cement-mill operation (intake -> advise -> govern
  ...
  decision to a human operator (the quality-control engineer). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/ship-cement-batch`/`:actuation/issue-mill-
  certificate` ALWAYS reach this node when the governor is clean --
  see `cementmill.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [cementmill.cementmilladvisor :as cementmilladvisor]
            [cementmill.governor :as governor]
...
(defn build
  ...
  `cementmill.store/Store`).
  opts:
    :advisor      -- a `cementmill.cementmilladvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (cementmilladvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Cement Mill Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (cementmilladvisor/-advise advisor store request)]
            {:proposal p :audit [(cementmilladvisor/trace request p)]})))

      ;; Kiln Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
